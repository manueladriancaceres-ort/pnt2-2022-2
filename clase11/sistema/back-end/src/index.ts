        ...
        jsonwebtoken.verify(token,'clave_secreta',(err:any, payload:any) => {
            if(err) {
                ...
                const producto: Producto = 
                    new Producto(Number(req.body.id),req.body.nombre,Number(req.body.importe));
...
app.get('/api/producto',  (req,res) => {
    ...
    if(req.headers['authorization']!==undefined) {
        ...
        const bearer = bearerToken.split(' ');
        const token = bearer[1];
        jsonwebtoken.verify(token,'clave_secreta', async (err:any, payload:any) => {
            if(err) {
                res.sendStatus(401);
            } else {
                console.log(payload);                
...
app.post('/api/login', async (req,res) => {
    ...
    if( req.body ) {
        ...
        if ( req.body.password == usuario.getPassword()) {
            const token = 
            jsonwebtoken.sign({email:usuario.getEmail(),rol:'adm'},'clave_secreta')
